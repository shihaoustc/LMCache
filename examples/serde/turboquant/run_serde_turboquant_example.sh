#!/usr/bin/env bash
#
# End-to-end example: LMCache MP server + disk L2 adapter + TurboQuant serde + vLLM.
#
# Flow:
#   1. Start `lmcache server` with:
#        - L1 (CPU) cache enabled
#        - L2 disk (fs) adapter
#        - TurboQuant serde on the L2 adapter
...
#   - 1 GPU available
#   - GPU + PyTorch + Triton with CUDA support
set -e
set -o pipefail
...
# ---------------------------------------------------------------------------
# Step 1: Launch lmcache MP server (CPU L1 + disk L2 with TurboQuant serde)
# ---------------------------------------------------------------------------
echo "============================================"
...
# ---------------------------------------------------------------------------
# Step 3: First inference — cold path (L1 -> L2 store with TurboQuant serialize)
# ---------------------------------------------------------------------------
...
echo "============================================"
echo "Expected: KV is computed, written to L1, then async-stored to L2 disk via TurboQuant serialize"
...
# ---------------------------------------------------------------------------
# Step 5: Re-run the same request — triggers L2 prefetch + TurboQuant deserialize
# ---------------------------------------------------------------------------
echo ""
echo "============================================"
...
echo "          TurboQuant deserialize back into KV, vLLM resumes from cache."
